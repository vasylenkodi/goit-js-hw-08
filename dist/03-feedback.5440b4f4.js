...
refs.form.addEventListener("input", (0, _lodashThrottleDefault.default)(storeDataToLocalStorage, 500));
refs.form.addEventListener("submit", clearForm);
function setEmailInputValue() {
    feedbackFormState.email = JSON.parse(localStorage.getItem("feedback-form-state")).email;
    refs.email.value = feedbackFormState[refs.email.name];
}
function setMessageInputValue() {
    feedbackFormState.message = JSON.parse(localStorage.getItem("feedback-form-state")).message;
